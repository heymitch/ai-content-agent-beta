...
async function bootstrap() {
  ...
  // Overall timeout: 30 seconds max for entire bootstrap process
  const BOOTSTRAP_TIMEOUT = 30000;
  const timeoutPromise = new Promise((_, reject) => {
    setTimeout(() => {
      reject(new Error(`Bootstrap timeout after ${BOOTSTRAP_TIMEOUT / 1000}s`));
    }, BOOTSTRAP_TIMEOUT);
  });
  ...
  try {
    ...
    log('✅', 'Bootstrap completed successfully', colors.green);
    console.log('');
  } catch (err) {
    if (err.message.includes('timeout')) {
      log('⏱️', `Bootstrap timed out after ${BOOTSTRAP_TIMEOUT / 1000}s`, colors.yellow);
      console.log('');
      console.log('⏭️  Continuing to start app anyway...');
      console.log('');
      console.log('💡 The app will try to connect to the database when needed.');
      console.log('   You can run bootstrap manually later: npm run bootstrap');
      console.log('');
    } else {
      log('⚠️', 'Bootstrap failed:', colors.yellow);
      console.error(err.message);
      console.log('');
      console.log('⏭️  Continuing to start app anyway...');
      console.log('');
