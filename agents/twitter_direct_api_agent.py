"""
Twitter Direct API Agent - Tier 2 Orchestrator
Uses direct Anthropic API calls instead of Claude Agent SDK to eliminate hanging issues.
Maintains same interface as twitter_sdk_agent.py for drop-in replacement.
"""
...
from tools.twitter_native_tools import (
...
# ================== TWITTER DIRECT API AGENT CLASS ==================

class TwitterDirectAPIAgent:
    """
    Tier 2: Twitter-specific agent using direct Anthropic API
    Drop-in replacement for TwitterSDKAgent without SDK hanging issues
    """
    ...
    def __init__(
        ...
        """Initialize Twitter Direct API Agent
        ...
        # Twitter-specific system prompt (keep it small for fast initialization)
        base_system_prompt = """You are a Twitter thread creation agent with a critical philosophy:
...
The user spent time thinking through their post. Your job is to make it BETTER, not to replace their thinking with generic AI output."""
        ...
        print("🎯 Twitter Direct API Agent initialized (no SDK, no hangs)")
    ...
    async def create_post(
        ...
        Args:
            ...
            thread_type: standard, carousel, or video
