"""
Email Direct API Agent - Tier 2 Orchestrator
Uses direct Anthropic API calls instead of Claude Agent SDK to eliminate hanging issues.
Maintains same interface as email_sdk_agent.py for drop-in replacement.
"""
...
from tools.email_native_tools import (
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "subject_line": {"type": "string", "description": "Selected subject line"},
                "context": {"type": "string", "description": "Additional context"}
            },
            "required": ["topic", "subject_line", "context"]
        }
    },
    {
...
class EmailDirectAPIAgent:
    """
    Tier 2: Email-specific agent using direct Anthropic API
    Drop-in replacement for EmailSDKAgent without SDK hanging issues
    """
    ...
    def __init__(
        ...
        """Initialize Email Direct API Agent
        ...
        base_system_prompt = """You are an Email content creation agent with a critical philosophy:
...
The user spent time thinking through their post. Your job is to make it BETTER, not to replace their thinking with generic AI output."""
        ...
        print("🎯 Email Direct API Agent initialized (no SDK, no hangs)")
    ...
    async def create_post(
        ...
        """
        Create an Email using direct Anthropic API with manual tool calling

        Args:
            topic: Main topic/angle
            context: Additional requirements, CTAs, etc.
            email_type: value, direct, indirect, weekly_update
        ...
        try:
            ...
            stacked_system = stack_prompts("email")
            ...
            if thinking_mode:
                ...
                workflow_section = f"""WORKFLOW (THINKING MODE - Higher Quality):
{word_count_guidance}
...
5. Return JSON with FIXED content and validation metadata:
   {{
     ...
     "gptzero_flagged_sentences": [flagged sentences]
                ...
                workflow_section = f"""WORKFLOW (ONE-SHOT):
{word_count_guidance}
...
3. Return JSON with the email and self-assessment:
   {{
     "subject_line": "...",
     "preview_text": "...",
     "email_body": "...",
     "self_score": [score from create_human_draft self_assessment]
            ...
            creation_prompt = f"""Create an email newsletter ({normalized_email_type} format).
...
Your goal: 18+/25 on the first pass. The stacked rules have everything you need."""
            ...
            log_error(logger, "Email Direct API Agent error", error=e, context=log_context)
            log_operation_end(
                logger,
                "create_email_post_direct_api",
