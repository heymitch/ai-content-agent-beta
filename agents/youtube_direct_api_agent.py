...
The user spent time thinking through their post. Your job is to make it BETTER, not to replace their thinking with generic AI output."""
    ...
    async def create_post(
        ...
        Args:
            ...
            script_type: standard, carousel, or video
        ...
        try:
            ...
            from prompts.youtube_tools import WRITE_LIKE_HUMAN_RULES

            # Build creation prompt with FULL writing rules (will be cached)
            creation_prompt = f"""{WRITE_LIKE_HUMAN_RULES}

═══════════════════════════════════════════════════════════
YOUTUBE SCRIPT CREATION TASK
═══════════════════════════════════════════════════════════

Create a YouTube video script ({script_type} format).
...
IF YES (rich strategic context):
   ...
   → DO call create_human_script WITH the outline as context
...
IF NO (topic or thin outline):
   ...
   → Call create_human_script to build full content (returns ONLY post_text, no scoring)
...
async def create_youtube_script_workflow(
    ...
    try:
        script_type = "carousel" if "visual" in style.lower() else "standard"
