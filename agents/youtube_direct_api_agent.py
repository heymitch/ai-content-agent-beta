"""
YouTube Direct API Agent - Tier 2 Orchestrator
Uses direct Anthropic API calls instead of Claude Agent SDK to eliminate hanging issues.
Maintains same interface as youtube_sdk_agent.py for drop-in replacement.
"""
...
async def execute_tool(tool_name: str, tool_input: Dict[str, Any]) -> str:
    ...
    try:
        ...
        elif tool_name == "create_human_script":
            result = await asyncio.wait_for(
                create_human_script_native(
                    topic=tool_input.get('topic', ''),
                    video_hook=tool_input.get('video_hook', ''),
                    context=tool_input.get('context', '')
                ),
                timeout=30.0
            )
...
class YouTubeDirectAPIAgent:
    """
    Tier 2: Email-specific agent using direct Anthropic API
    Drop-in replacement for YouTubeSDKAgent without SDK hanging issues
    """
    ...
    def __init__(
        ...
        """Initialize YouTube Direct API Agent
        ...
        # YouTube-specific system prompt (keep it small for fast initialization)
        base_system_prompt = """You are a YouTube script creation agent with a critical philosophy:
...
IF the user gave you just a topic or thin outline (<200 words, no narrative):
   ...
   → create_human_script to build full content
...
The user spent time thinking through their post. Your job is to make it BETTER, not to replace their thinking with generic AI output."""
        ...
        print("🎯 YouTube Direct API Agent initialized (no SDK, no hangs)")
    ...
    async def create_post(
        ...
        """
        Create a YouTube script using direct Anthropic API with manual tool calling

        Args:
            topic: Main topic/angle
            context: Additional requirements, CTAs, etc.
            script_type: standard, tutorial, or explainer
        ...
        try:
            ...
            stacked_system = stack_prompts("youtube")
...
2. Call tools to create draft:
   ...
   - create_human_script (always - pass context through)
...
3. Return JSON with the script and self-assessment:
   {
     "title": "...",
     "script": "...",
     "description": "...",
     "self_score": [score from create_human_script self_assessment]
   }"""

            # Build the creation prompt
            creation_prompt = f"""Create a YouTube video script ({script_type} format).
...
Your goal: 18+/25 on the first pass. The stacked rules have everything you need."""
            ...
            if thinking_mode:
                ...
                logger.info(f"🔧 Default mode: providing {len(tools_to_use)} tools (excluding validation tools)")
            ...
            log_error(logger, "YouTube Direct API Agent error", error=e, context=log_context)
            log_operation_end(
                logger,
                "create_youtube_script_direct_api",
